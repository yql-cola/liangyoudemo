...
class PurchaseInControllerTest {
    ...
    @Test
    void updatePurchaseInReturnsSuccess() throws Exception {
        PurchaseInService purchaseInService = new PurchaseInService(null, null, null, null, null) {
            @Override
        ...
        item.setProductId(1L);
        item.setUnitId(1L);
        item.setQty(new BigDecimal("8"));
        item.setUnitPrice(new BigDecimal("100"));
        item.setAmount(new BigDecimal("800"));
        item.setBatchNo("B002");
        item.setInboundTime(LocalDateTime.now());
        ...
        mockMvc.perform(put("/api/v1/purchase-ins/200")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isOk())
